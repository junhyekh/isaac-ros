...
ROBOT_ID = 1 # change this to your robot id
...
class G1ObservationSubscriber(Node):
    ...
    def compose_observation(self, cmd, prev_action, base_ang_vel, base_quat, recieved_joint_names, joint_pos, joint_vel, simtime) -> np.ndarray:
        ...
        omega = base_ang_vel * self.ang_vel_scale
        ...
        for i, name in enumerate(self.walking_joint_names):
            walking_joint_indices.append(recieved_joint_names.index(name))
        walking_joint_indices = np.array(walking_joint_indices)

        qj = np.array(joint_pos)[walking_joint_indices]
        dqj = np.array(joint_vel)[walking_joint_indices]

        qj = (qj - self.default_angles) * self.dof_pos_scale
        dqj = dqj * self.dof_vel_scale

        # --- 4) phase signal (sin, cos) ---
        phase = (simtime % self.period) / self.period
    ...
    def get_observation(self, cmd, prev_action)-> np.ndarray:
        if self.joint_msg is None:
            return
        recieved_joint_names = list(copy.deepcopy(self.joint_msg.name))
    ...
    def match_joint_order(self, action: np.ndarray):
        ...
        return full_names, full_positions
    ...
    try:
        ...
        while True:
            ...
            if node.simtime - step_start > 0.02:
                step_start = node.simtime
                obs = node.get_observation(cmd, action)
                if obs is None:
                    continue
